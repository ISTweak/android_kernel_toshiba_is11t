...
#include <linux/kthread.h> /* FUJITSU:2011-06-08 GENERIC for media key */
...
/* FUJITSU:2011-07-07 USB start */
	...
	K_CABLE_CRADLE_CONNECT = 6,	/*The notice of AC Wall charger connect*/
	K_CABLE_CRADLE_DISCONNECT = 7	/*The notice of AC Wall charger disconnect*/
/* FUJITSU:2011-07-07 USB end */
        /* And mode ... */
};

/* FUJITSU:2011-07-16 USB_GB del start */
#if 0
extern void msm_usb_phy_reg_init(void);
extern void msm_otg_enable_irq(void);
extern void msm_otg_disable_irq(void);
extern int g_cradle_stat; /* enable 1:disable 0 */
extern int g_cradle_otg_charge_type;
#endif
/* FUJITSU:2011-07-16 USB_GB del end */
/* FUJITSU:2011-07-09 USB end */

/* FUJITSU:2011-06-15 GANERIC mod ChargeError start */
...
/* FUJITSU:2011-06-15 GENERIC mod ChargeError end */
...
/* FUJITSU:2011-06-08 GENERIC for media key start */
static struct task_struct *media_key_task = NULL;
static int mediakeyloop = 0;
static bool mediakeyflg = false;

static int check_start_media_key(void * unused){
	while(1){
		mdelay(100);
		if(mediakeyloop > 0) mediakeyloop--;
		else break;
	}
	mediakeyloop = 0;
	return 0;
}
/* FUJITSU:2011-06-08 GENERIC for media key end */
/* FUJITSU:2011-04-08 GENERIC for media key start */
static bool ena_media_key = false;

void enable_media_key(bool enable)
{
	ena_media_key = enable;
/* FUJITSU:2011-06-08 for media key start */
	if(enable){
		if(mediakeyloop == 0) {
			mediakeyloop = 5;
			media_key_task = kthread_run(check_start_media_key, NULL, "mediakey");
		} else {
			mediakeyloop = 5;
		}
	}else{
/* FUJITSU:2011-07-01 for media key2 start */
        input_report_key(hs->ipdev, KEY_MEDIA, false);
/* FUJITSU:2011-07-01 for media key2 start */
		mediakeyloop = 0;
		mediakeyflg = false;
	}
...
EXPORT_SYMBOL(enable_media_key);
...
/* FUJITSU:2011-06-15 GENERIC mod ChargeError start */
...
/* FUJITSU:2011-07-13 USB_GB del start */
#if 0
...
#endif
/* FUJITSU:2011-07-13 USB_GB del end */
...
/* FUJITSU:2011-07-16 USB_GB del end */
		default:
/* FUJITSU:2011-07-16 USB_GB add start */
			printk(KERN_DEBUG "%s: USB_CABLE_DET unknown key param = %d \n",__func__,key_parm);
/* FUJITSU:2011-07-16 USB_GB add end */
			/* error !! */
			break;
		}
		return;
...
/* FUJITSU:2011-07-01 for media key2 end */
		break;
/* FUJITSU:2011-04-08 for media key end */
