...
/* FUJITSU:2011_04_16 add bsp-sensor start */
#include <linux/walkmotion.h>
#include <linux/i2cuart.h>
/* FUJITSU:2011_04_16 add bsp-sensor end */
...
#include "devices.h"
#include "timer.h"
/* FUJITSU:2011-03-15 Add for F11EIF HW start */
#include "msm-keypad-devices.h"
/* FUJITSU:2011-03-15 Add for F11EIF HW end */
#ifdef CONFIG_USB_ANDROID
#include <linux/usb/android_composite.h>
/* FUJITSU:2011-04-22 USB start */
#include "proc_comm.h"
/* FUJITSU:2011-04-22 USB end */
#endif
#include "pm.h"
#include "spm.h"
...
/* FUJITSU:2011-04-11 del BMA150 start */
#if 0
#include <linux/bma150.h>
#endif
/* FUJITSU:2011-04-11 del BMA150 end */
...
/* FUJITSU:2011-06-21 change pmem size up start */
#define MSM_PMEM_SF_SIZE	0x2000000
//#define MSM_PMEM_SF_SIZE	0x1700000
/* FUJITSU:2011-06-21 change pmem size up end */

#define MSM_FB_SIZE		0x500000
/* FUJITSU:2011-05-19 modify pmem adsp size change start */
//#define MSM_PMEM_ADSP_SIZE      0x1800000
#define MSM_PMEM_ADSP_SIZE        0x1C00000
/* FUJITSU:2011-05-19 modify pmem adsp size change end */
...
/* FUJITSU:2011-02-22 del OPTNAV start */
#if 0
...
#endif
/* FUJITSU:2011-02-22 del OPTNAV end */
...
/* FUJITSU:2011-05-01 VIB GPIO port start */
#if 0
#define PMIC_GPIO_HAP_ENABLE   16  /* PMIC GPIO Number 17 */
#else
#define PMIC_GPIO_HAP_ENABLE   14  /* PMIC GPIO Number 15 */
#endif
#define HAP_PWM_GPIO 16 
/* FUJITSU:2011-05-01 VIB GPIO port end */
/* for ISA1200  */
#define HAP_LVL_SHFT_MSM_GPIO 17

/* FUJITSU:2011-04-11 del WLAN_EXT_POR start */
#if 0
#define PMIC_GPIO_WLAN_EXT_POR  22 /* PMIC GPIO NUMBER 23 */
#endif
/* FUJITSU:2011-04-11 del WLAN_EXT_POR end */
...
/* FUJITSU:2011-05-25 Add  bsp-sensor start */
#define KO_UART4_NIRQ 7
/* FUJITSU:2011-05-25 Add  bsp-sensor end */

/* FUJITSU:2011-06-08 vib setting start */
#if 0
int vib_set_flag;
EXPORT_SYMBOL(vib_set_flag);
#endif
/* FUJITSU:2011-06-08 vib setting end */
...
/* FUJITSU:2011-04-14 LED start */
	struct pm8058_gpio keybacklight = {
		.direction      = PM_GPIO_DIR_OUT,
		.pull           = PM_GPIO_PULL_NO,
		.vin_sel        = PM_GPIO_VIN_S3,
		.function       = PM_GPIO_FUNC_NORMAL,
...
/* FUJITSU:2011-05-25 bsp-sensor Add start */
...
/* FUJITSU:2011-05-25 bsp-sensor Add end */
...
/* FUJITSU:2011-05-03 VIB start */
	rc = pm8058_gpio_config(PMIC_GPIO_HAP_ENABLE, &haptics_enable);
	if (rc) {
		printk("[hap_log]:%s PMIC GPIO %d write failed\n",
				__func__,(PMIC_GPIO_HAP_ENABLE + 1));
		return rc;
	}

	rc = pm8058_gpio_config(18, &haptics_enable);
	if (rc) {
		printk("[hap_log]:%s PMIC GPIO %d write failed\n",
						__func__,19);
		return rc;
	...
	if(0x0B <= system_rev) {
		rc = pm8058_gpio_config(23, &haptics_enable);
		if (rc) {
			printk("[hap_log]:%s PMIC GPIO %d write failed\n",
				__func__,23);
			return rc;
		...
		rc = gpio_tlmm_config(GPIO_CFG(HAP_PWM_GPIO, 0,
						GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
						GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		if (rc) {
			pr_err("[hap log]%s: Could not configure gpio %d\n",
					__func__, HAP_PWM_GPIO);
		}
	}
/* FUJITSU:2011-05-03 VIB end */
...
/* FUJITSU:2011-04-14 LED start */
	rc = pm8058_gpio_config(PMIC_GPIO_KEYLIGHT -1, &keybacklight);
	if (rc) {
...
/* FUJITSU:2011/02/23  No virtual Key start */
#if 0
/*virtual key support */
...
#endif
/* FUJITSU:2011/02/23  No virtual Key end */

/* FUJITSU:2011/03/03 IRQ(150) INT(18) mod start */
#if 0
...
//        buf[1] = 0x0C;  /* 13H: LDO4 LDO3 ON  */
        buf[1] = 0x0F;  /* 13H: LDO1-4 ON  */   /* FUJITSU:2011-06-06 bootlogo blackout chg */
    ...
    printk("[DD]:I2C init[O]\n");
	return 0;
}

static int cyttsp_i2c_wakeup(void)
{
/* FUJITSU:2011/03/29 mod start */ 
    printk("[DD]:I2C wakeup[I]\n");

	/* Reset Lo change Reset->Int */
    gpio_set_value(CY_I2C_IRQ_GPIO, 0);

    /* FUJUTSU:2011/03/29 20->1 ms wait reset Lo->Hi */
    mdelay(1);

    /* Reset Hi change Reset->Int */
    gpio_set_value(CY_I2C_IRQ_GPIO, 1);

	/* FUJUTSU:2011/03/29 INT set input */
	gpio_direction_input(CY_I2C_IRQ_GPIO);

	printk("[DD]:I2C wakeup[O]\n");
/* FUJITSU:2011/03/29 mod end */ 

    return 0;
}

/* FUJITSU:2011/05/31 mod start */ 
static int cyttsp_xres_wakeup(void)
{
    printk("[DD]:I2C xres_wakeup[I]\n");
    /* Reset Hi */
    gpio_set_value(CY_I2C_XRST_GPIO, 1);

    /* 20ms wait reset Lo->Hi */
    mdelay(1);
...
static struct cyttsp_platform_data cypress_i2c_ttsp_platform_data = {
	...
	.wakeup_xres = cyttsp_xres_wakeup, /* FUJITSU:2011/04/27 add */
	.init = cyttsp_i2c_init,
	.mt_sync = input_mt_sync,   /* FUJITSU:2011/03/06 add */
	.maxx = 479,
	.maxy = 799,                /* FUJITSU:2011/03/24 mod */
	.flags = 0x00,              /* FUJITSU:2011/03/24 mod */
	.gen = CY_GEN3,
	.use_st = 0,
	.use_mt = 1,
	.use_trk_id = 1,
	...
	.gest_set = CY_GEST_GRP1 | CY_GEST_GRP2 |
				CY_GEST_GRP3 | CY_GEST_GRP4 |
				CY_ACT_DIST,
    .scn_typ = 0x01,            /* FUJITSU:2011/03/09 mod */
	.act_intrvl = 10,			/* FUJITSU:2011/03/11 00->10 mod */
	.tch_tmout = 100,			/* FUJITSU:2011/03/11 100->100 mod */
	.lp_intrvl = 20,			/* FUJITSU:2011/03/29 50->20 mod */
	.name = CY_I2C_NAME,
	.irq_gpio = CY_I2C_IRQ_GPIO,
};
/* FUJITSU:2011/03/03 IRQ(150) INT(18) mod end  */
...
/* FUJITSU:2011-04-14 LED start */
static struct pmic8058_led pmic8058_f11eif_leds[] = {
	[0] = {
		.name		= "button-backlight",
		.max_brightness = 1,
		.id		= PMIC8058_ID_LED_BT_LIGHT,
	},
	[1] = {
		.name		= "red",
...
static struct pmic8058_leds_platform_data pm8058_f11eif_leds_data = {
	.num_leds = ARRAY_SIZE(pmic8058_f11eif_leds),
	.leds	= pmic8058_f11eif_leds,
};
/* FUJITSU:2011-04-14 LED end */
...
#endif
/* FUJITSU:2011_04_06 add camera start */
#if defined(CONFIG_MACH_F11EIF)
	{
		I2C_BOARD_INFO("m6mo_cam", 0x1F),
	},
#endif
/* FUJITSU:2011_04_06 add camera end */
};
...
/* FUJITSU:2011_04_06 mod camera start */
//	.ioclk.mclk_clk_rate = 24000000,
	.ioclk.mclk_clk_rate = 24576000,
/* FUJITSU:2011_04_06 mod camera end */
...
static struct msm_camera_sensor_flash_data flash_f11eif = {
//	.flash_type = MSM_CAMERA_FLASH_LED,
	.flash_type = MSM_CAMERA_FLASH_NONE,
	.flash_src  = &msm_flash_src_pwm
};

static struct msm_camera_sensor_info msm_camera_sensor_f11eif_data = {
	.sensor_name    = "f11eif",
	.sensor_reset   = 0,
	.sensor_pwd     = 85,
	.vcm_pwd        = 1,
	...
	.resource       = msm_camera_resources,
	.num_resources  = ARRAY_SIZE(msm_camera_resources),
	.flash_data     = &flash_f11eif,
	.csi_if         = 1
};

static struct platform_device msm_camera_sensor_f11eif = {
	.name      = "msm_camera_f11eif",
	.dev       = {
		.platform_data = &msm_camera_sensor_f11eif_data,
	},
};
#endif
/* FUJITSU:2011_04_06 add camera end */
...
/* FUJITSU:2011-02-22 del TIMPANI start */
#if 0
#define TIMPANI_RESET_GPIO	1
#endif
/* FUJITSU:2011-02-22 del TIMPANI end */
...
/* FUJITSU:2011-02-22 del FM start */
#if 0
...
/* FUJITSU:2011-02-22 del TSADC start */
#if 0
...
#endif
/* FUJITSU:2011-02-22 del TSADC end */
...
/* FUJUTSU:2011_04_20 add bsp-sensor start */
...
/* FUJITSU:2011-05-25 Mod bsp-sensor start */
...
/* FUJUTSU:2011_04_20 add bsp-sensor end */
...
/* FUJITSU:2011-06-02 USB setteing start */
    /* Normal Mode                                                      */
    /* MSC */
    {
        .product_id     = 0x12AB,
...
/* FUJITSU:2011-05-19 modify disable idle power collapse start */
//	[MSM_PM_SLEEP_MODE_POWER_COLLAPSE].idle_enabled = 1,
	[MSM_PM_SLEEP_MODE_POWER_COLLAPSE].idle_enabled = 0,    //CHECK: temporary change value 1 to 0
/* FUJITSU:2011-05-19 modify disable idle power collapse end */
...
//	[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_NO_XO_SHUTDOWN].idle_enabled = 1,
	[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_NO_XO_SHUTDOWN].idle_enabled = 0,     //CHECK: temporary change value 1 to 0
/* FUJITSU:2011-05-19 modify disable idle power collapse end */
...
#else
/* FUJITSU:2011-04-16 add WAVEIF start */
static struct spi_board_info lcdc_sharp_spi_board_info[] __initdata = {
	{
...
/* FUJITSU:2011-04-16 add WAVEIF end */
#endif
#endif

/* FUJITSU:2011-06-11 delete SPI GPIO start */
#if 0
static struct msm_gpio qsd_spi_gpio_config_data[] = {
	...
	{ GPIO_CFG(48, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_miso" },
};
...
#endif
/* FUJITSU:2011-06-11 delete SPI GPIO end */
...
/* FUJITSU:2011-04-27 del FeliCa start */
#if 0
...
#endif
/* FUJITSU:2011-04-27 del FeliCa end */
...
/* FUJITSU:2011-06-22 PanelPowerControl start */
static bool display_on_status = true;	//	paneloff
/* FUJITSU:2011-06-22 PanelPowerControl end */
...
/* FUJITSU:2011-06-22 PanelPowerControl start */
		...
		if(display_on_status != true)
		{
			...
			//client register address
			buffer[0] = 0x13;
			//data LDO1 and LD03 and LD04 [ON]
			buffer[1] = 0x0D;
			ret = i2c_transfer(i2c_lcd, &msg, 1);
			if(ret < 0){
				pr_err("[DD](%s):I2C ERROR [%d] ret = %d\n",__func__, __LINE__, ret);
...
/* FUJITSU:2011-05-02 bootbacklight ctrl start */
        if(!backlight_init){
            ...
            buffer[0] = 0x03;
            //data  20mA
//            buffer[1] = 0x63;/* FUJITSU:2011-06-06 bootbacklight brightness chg */
            buffer[1] = 0x30;  /* FUJITSU:2011-06-06 bootbacklight brightness chg */
            ret = i2c_transfer(i2c_lcd, &msg, 1);
           if(ret < 0){
                pr_err("[DD](%s):I2C ERROR [%d] ret = %d\n",__func__, __LINE__, ret);
                return ret;
...
#if 0
/* FUJITSU:2011-04-14 backlight power off (temporary hack) - start */
    else {
        //client register address
        buffer[0] = 0x02;
        //data  MLEDEN [OFF]
...
/* FUJITSU:2011-04-14 backlight power off (temporary hack) - end */
#endif
/* FUJITSU:2011-06-22 PanelPowerControl start */
#if 1	//	
	else
...
/* FUJITSU:2011-03-03 display power end */
/* FUJITSU:2011-03-03 del display power start */
#if 0
...
#endif
/* FUJITSU:2011-03-03 del display power end */
...
/* FUJITSU:2011-06-23 change mdp_core_clk_rate start */
/*	122880000,
	122880000,
	122880000,
	192000000,	*/
	...
	192000000,
	192000000,
/* FUJITSU:2011-06-23 change mdp_core_clk_rate end */
};
...
void set_power_wlan_bt(int on, int is_wlan)
{
	...
	if ( on )
	{
		printk(KERN_ERR "[Eiffel] BT/WLAN-ON \n");
	...
	{
		printk(KERN_ERR "[Eiffel] BT/WLAN-OFF \n");
		if ( ( wlan_power_flg ) || ( bt_power_flg ) )
		{
			if ( is_wlan )
			{
...
int wlan_power(int on)
{
	...
	if (on) {
		...
		for (pin = 0; pin < ARRAY_SIZE(bt_config_power_on); pin++) {
			...
			if (rc) {
				printk(KERN_ERR
					"[Eiffel]%s: Err gpio_tlmm_config(%#x)=%d\n",
...
int pin,rc;
		...
		rc = gpio_request(96, "[Eiffel]KO_WLAN_BT_PWR_ONOFF");
		if (rc) {
			printk(KERN_ERR "[Eiffel] Err gpio_request KO_WLAN_BT_PWR_ONOFF\n");
		}
		rc = gpio_request(39, "[Eiffel]KO_WLAN_PWON");
		if (rc) {
			printk(KERN_ERR "[Eiffel] Err gpio_request KO_WLAN_PWON\n");
		}
		rc = gpio_request(38, "[Eiffel]KO_WLAN_RESET");
		if (rc) {
			printk(KERN_ERR "[Eiffel] Err gpio_request KO_WLAN_RESET\n");
		}
...
#if 0
// GPIO SETTING DISABLE
static void bt_gpio_request(void)
{
	int rc;

	rc = gpio_request(138, "KO_BT_PCMOUT");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_PCMOUT request failed\n",__func__);
	}

	rc = gpio_request(139, "KO_BT_PCMIN");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_PCMIN request failed\n",__func__);
		}

	rc = gpio_request(140, "KO_BT_PCM_FSYNC");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_PCM_FSYNC request failed\n",__func__);
		}

	rc = gpio_request(141, "KO_BT_PCM_DCLK");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_PCM_DCLK request failed\n",__func__);
		}

	rc = gpio_request(147, "KO_BT_HOSTWAKEUP");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_HOSTWAKEUP request failed\n",__func__);
	}

	rc = gpio_request(21, "KO_BT_WAKEUP");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_WAKEUP request failed\n",__func__);
	}

	rc = gpio_request(41, "KO_BT_RESET");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_RESET request failed\n",__func__);
	}

	rc = gpio_request(43, "KO_BT_PWON");
	if (rc) {
		printk(KERN_ERR "%s: KO_BT_PWON request failed\n",__func__);
	}

	rc = gpio_request(96, "KO_WLAN_BT_PWR_ONOFF");
	if (rc) {
		printk(KERN_ERR "%s: KO_WLAN_BT_PWR_ONOFF request failed\n",__func__);
	}
}

static void bt_pmic_gpio_request(void)
{
	int rc;
	// TX
	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_TXD_21), "KO_PMICUART1_BT_TXD_21");
	if (rc) {
		printk(KERN_ERR "%s: KO_PMICUART1_BT_TXD request failed\n",__func__);
	}

	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_TXD_22), "KO_PMICUART1_BT_TXD_22");
	if (rc) {
		printk(KERN_ERR "%s: gpio_request KO_PMICUART1_BT_RXD request failed\n",__func__);
	}

	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_TXD_23), "KO_PMICUART1_BT_TXD_23");
	if (rc) {
		printk(KERN_ERR "%s: gpio_request KO_PMICUART1_BT_RXD request failed\n",__func__);
	}

	// RX
	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_RXD_33), "KO_PMICUART1_BT_RXD_33");
	if (rc) {
		printk(KERN_ERR "%s: KO_PMICUART1_BT_TXD request failed\n",__func__);
	}

	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_RXD_34), "KO_PMICUART1_BT_RXD_34");
	if (rc) {
		printk(KERN_ERR "%s: gpio_request KO_PMICUART1_BT_RXD request failed\n",__func__);
	}

	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_RXD_35), "KO_PMICUART1_BT_RXD_35");
	if (rc) {
		printk(KERN_ERR "%s: gpio_request KO_PMICUART1_BT_RXD request failed\n",__func__);
	}

	// PMIC
	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_PMIC_TXD_36), "KO_PMICUART1_BT_PMIC_TXD_36");
	if (rc) {
		printk(KERN_ERR "%s: gpio_request KO_PMICUART1_BT_PMIC_TXD request failed\n",__func__);
		}

	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(KO_PMICUART1_BT_PMIC_RXD_37), "KO_PMICUART1_BT_PMIC_RXD_37");
	if (rc) {
		printk(KERN_ERR "%s: gpio_request KO_PMICUART1_BT_PMIC_RXD request failed\n",__func__);
	}
...
/* FUJITSU:2011_04_06 add camera end */
	...
	&msm_ebi1_thermal,
    &memread_dev,
/* FUJITSU:2011/04/16 add bsp-sensor start */
	&fj_walkmotion_device,
	&i2cuart_device
/* FUJITSU:2011/04/16 add bsp-sensor end */
};
...
/* FUJITSU:2011/06/23 I2C Clock 320K start */
...
/* FUJITSU:2011/06/23 I2C Clock 320K end */
...
/* FUJITSU:2011/05/18 add bsp-sensor start */
...
/* FUJITSU:2011/05/18 add bsp-sensor end */
...
/* FUJITSU:2011/05/18 add bsp-sensor end */
#if 0   /* FUJITSU:2011/05/6 add bsp-sensor del start */
	gpio_set_value( 104, 0 );
	udelay(100);
	gpio_set_value( 104, 1 );
	udelay(100);
	ret = gpio_get_value( 104 );

	printk( KERN_INFO "%s: gpio_get_value(104) = [%d] \n",__func__, ret );
#endif  /* FUJITSU:2011/05/6 add bsp-sensor del end */
...
/* FUJITSU:2011-07-01 mod SDCard start */
...
/* FUJITSU:2011-07-01 mod SDCard end */
};
...
{
/* FUJITSU:2011-07-09 mod SDCard start */
	struct vreg *vreg_mmc_sdio;
	int rc;
/* FUJITSU:2011-07-09 mod SDCard end */
...
/* FUJITSU:2011-07-09 mod SDCard start */
	vreg_mmc_sdio = vreg_get(NULL, "mmc");
	if(!vreg_mmc_sdio){
		printk(KERN_ERR "%s: VREG L5 get failed\n", __func__);
		return;
	}
	rc = vreg_set_level(vreg_mmc_sdio, 2850);
	if(rc){
		printk(KERN_ERR "%s: VREG L5 set failed\n", __func__);
		return;
	}
	rc = vreg_enable(vreg_mmc_sdio);
	if(rc){
		printk(KERN_ERR "%s: VREG L5 enable failed\n", __func__);
		return;
	}
/* FUJITSU:2011-07-09 mod SDCard end */
}
...
#endif
/* FUJITSU:2011-02-22 del end */
/* FUJITSU:2011-05-10 USB start */
extern int msm_usb_get_serial_number(void);
/* FUJITSU:2011-05-10 USB end */
...
/* FUJITSU:2011-05-13 modify for keypad start */
	gpio_tlmm_config( GPIO_CFG(115, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE );
	gpio_tlmm_config( GPIO_CFG(114, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE );
	gpio_tlmm_config( GPIO_CFG(113, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE );
	gpio_tlmm_config( GPIO_CFG( 67, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE );
	gpio_tlmm_config( GPIO_CFG( 69, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE );
/* FUJITSU:2011-05-13 modify for keypad end */
...
/* FUJITSU:2011-03-15 Add for F11EIF HW start */
	platform_device_register(&keypad_device_f11eif); 
/* FUJITSU:2011-03-15 Add for F11EIF HW end */

	pmic8058_leds_init();

/* FUJITSU:2011/02/22 del TMA300 start */
#if 0
	if (machine_is_msm7x30_fluid()) {
...
#endif
/* FUJITSU:2011/02/22 del TMA300 end */
	if (machine_is_msm8x55_svlte_surf() || machine_is_msm8x55_svlte_ffa()) {
...
/* FUJITSU:2011-06-02 USB start */
static void msm_read_nv_imei(void);
static void msm_read_nv_imei()
{
	...
	static unsigned char nv_imei_read[MSM_NV_UE_IMEI_SIZE] = {0};
	int length = 0,j = 5; 
	char temp_arr[7] = {0};

	dataread = (unsigned char *)smem_alloc_vendor1(SMEM_OEM_001);
	if(dataread == NULL)
	{
		memset(android_usb_pdata.product_serial_number,0,MSM_SERIAL_NUMBER_SIZE);
		strcpy(android_usb_pdata.product_serial_number,"F12C000000");
	printk( "%s: product_serial_number NULL [%s]\n", __func__, android_usb_pdata.product_serial_number);
		return;
	}
	...
	if ( imei_bcd_len == 0 )
	{
		memset(android_usb_pdata.product_serial_number,0,MSM_SERIAL_NUMBER_SIZE);
		strcpy(android_usb_pdata.product_serial_number,"F12C000000");
		printk( "%s: product_serial_number len0[%s]\n", __func__, android_usb_pdata.product_serial_number);
	}
	else if ( imei_bcd_len <= ( MSM_NV_UE_IMEI_SIZE - 1 ) )
	{
		...
		memset(android_usb_pdata.product_serial_number,0,MSM_SERIAL_NUMBER_SIZE);
		strcpy(android_usb_pdata.product_serial_number,"F12C");
		...
		temp_arr[6]='\0';
		printk( "%s: product_serial_number [%s]\n", __func__, android_usb_pdata.product_serial_number);
		strcat(android_usb_pdata.product_serial_number,temp_arr);
		printk( "%s: product_serial_number set [%s]\n", __func__, android_usb_pdata.product_serial_number);
	}
	...
	else
	{
		memset(android_usb_pdata.product_serial_number,0,MSM_SERIAL_NUMBER_SIZE);
		strcpy(android_usb_pdata.product_serial_number,"F12C000000");
		printk( "%s: product_serial_number invalid IMEI[%s]\n", __func__, android_usb_pdata.product_serial_number);
	}
}
...
int msm_usb_read_nvitem(unsigned int id, unsigned int *data)
{
	...
	/* read NV*/
/* FUJITSU:2011-05-09 USB start */
	rc = msm_proc_comm(PCOM_OEM_011, &id,(unsigned *)data);
/* FUJITSU:2011-05-09 USB end */
	if(rc)
...
/* FUJITSU:2011-05-25 PROXIMITY setting start */
...
/* FUJITSU:2011-05-25 PROXIMITY setting end */

/* FUJITSU:2011-06-12 vib setting start */
#if 0
void vib_set_on_off_flag(int flag)
{
	if(0x0B <= system_rev) {
		vib_set_flag = 0;
		//printk("[hap log]2-1 vib flag = %d \n",vib_set_flag);
	}
	else if(flag == 1){
		vib_set_flag++;
		//printk("[hap log]vib flag++ -> %d \n",vib_set_flag);
   		gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(14), 0);
		if (gpio_tlmm_config(GPIO_CFG(16, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE)){
			pr_err("%s: gpio_tlmm_config (gpio=%d) failed\n",
			       __func__, HAP_LVL_SHFT_MSM_GPIO);
		}
		gpio_set_value(16, 0);
		gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(18), 0);
	}else{
		vib_set_flag--;
		//printk("[hap log]vib flag-- -> %d \n",vib_set_flag);
	}
}
EXPORT_SYMBOL(vib_set_on_off_flag);
#endif
/* FUJITSU:2011-06-12 vib setting end */

/* FUJITSU:2011-02-17 add hardware start */
MACHINE_START(F11EIF, "F11EIF")
...
/* FUJITSU:2011-02-21 del hardware start */
#if 0
...
MACHINE_END
#endif
/* FUJITSU:2011-02-21 del hardware end */
