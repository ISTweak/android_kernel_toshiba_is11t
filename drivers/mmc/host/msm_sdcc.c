...
/* FUJITSU:2011-04-25 mod SDCard start */
#if 0
		host->eject = !status;
#else
		host->eject = status;
#endif
/* FUJITSU:2011-04-25 mod SDCard end */
...
static void msmsdcc_late_resume(struct early_suspend *h)
{
	...
	if (host->polling_enabled) {
		spin_lock_irqsave(&host->lock, flags);
		host->mmc->caps |= MMC_CAP_NEEDS_POLL;
/* FUJITSU:2011-06-02 EIFFEL START */
#if 0
		mmc_detect_change(host->mmc, 0);
#else
/* FUJITSU:2011-07-13 EIFFEL START */
// 2011.10.25 Fujitsu Del		mmc_detect_change(host->mmc, msecs_to_jiffies(4000));
		mmc_detect_change(host->mmc, msecs_to_jiffies(7000));// 2011.10.25 Fujitsu Add
/* FUJITSU:2011-07-13 EIFFEL END */
#endif
/* FUJITSU:2011-06-02 EIFFEL END */
		spin_unlock_irqrestore(&host->lock, flags);
	}
};
#endif
