...
/* FUJITSU:2011-07-29 for FOTA MOD-S */
#if 0
static unsigned cfg0_back;
static uint32_t ecc_buf_cfg_back;
/* FUJITSU:2011-06-25 for FOTA ADD-E */
#endif
static unsigned CFG0_A;
/* FUJITSU:2011-07-29 for FOTA MOD-E */
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	unsigned cwperpage;
	unsigned CFG0;			/* FUJITSU:2011-07-29 for FOTA ADD */
	uint32_t ecc_buf_cfg;	/* FUJITSU:2011-07-29 for FOTA ADD */
	unsigned partinfo = (apps_partition_offset > from) ? 1 : 0; /* FUJITSU:2011-06-25 for FOTA ADD */

/* FUJITSU:2011-07-29 for FOTA ADD-S */
    if ( partinfo ) { CFG0 = CFG0_M; ecc_buf_cfg = 0x1FF; }
    else {CFG0 = CFG0_A; ecc_buf_cfg = 0x203;}
/* FUJITSU:2011-07-29 for FOTA ADD-E */
#if VERBOSE
	pr_info("================================================="
			"================\n");
	...
	if (chip->CFG1 & CFG1_WIDE_FLASH)
		oob_col >>= 1;
/* FUJITSU:2011-07-29 for FOTA DEL-S */
#if 0
/* FUJITSU:2011-06-25 for FOTA ADD-S */
    if ( partinfo )
    {
...
/* FUJITSU:2011-06-25 for FOTA ADD-E */
#endif
/* FUJITSU:2011-07-29 for FOTA DEL-E */
	...
	while (page_count-- > 0) {
		...
		if (ops->mode != MTD_OOB_RAW) {
			...
			dma_buffer->data.cfg0 =
/*			(chip->CFG0 & ~(7U << 6))	  *//* FUJITSU:2011-07-29 for FOTA DEL */
			(CFG0 & ~(7U << 6))				/* FUJITSU:2011-07-29 for FOTA ADD */
...
/*				dma_buffer->data.ecccfg = chip->ecc_buf_cfg;  *//* FUJITSU:2011-07-29 for FOTA DEL */
				dma_buffer->data.ecccfg = ecc_buf_cfg;			/* FUJITSU:2011-07-29 for FOTA ADD */
				cmd->cmd = 0;
				cmd->src = msm_virt_to_dma(chip,
...
/* FUJITSU:2011-08-10 change ecc error check start */
#if 0
					if (ecc_errors > 1)
#else
					if (ecc_errors > 4)
#endif
/* FUJITSU:2011-08-10 change ecc error check end */
						pageerr = -EUCLEAN;
				}
			}
		}
...
/*				dma_buffer->data.cfg0 = (chip->CFG0 & *//* FUJITSU:2011-07-29 for FOTA DEL */
				dma_buffer->data.cfg0 = (CFG0 &			/* FUJITSU:2011-07-29 for FOTA ADD */
							~(7U << 6));
			} else {
/*				dma_buffer->data.cfg0 = (chip->CFG0 & *//* FUJITSU:2011-07-29 for FOTA DEL */
				dma_buffer->data.cfg0 = (CFG0 &			/* FUJITSU:2011-07-29 for FOTA ADD */
...
msm_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	unsigned CFG0;			/* FUJITSU:2011-07-29 for FOTA ADD */
	uint32_t ecc_buf_cfg;	/* FUJITSU:2011-07-29 for FOTA ADD */
	unsigned partinfo = (apps_partition_offset > to) ? 1 : 0;  /* FUJITSU:2011-06-25 for FOTA ADD */
...
/* FUJITSU:2011-07-29 for FOTA DEL-E */
	...
	while (page_count-- > 0) {
		...
		if (ops->mode != MTD_OOB_RAW) {
/*			dma_buffer->data.cfg0 = ((chip->CFG0 & ~(7U << 6))    *//* FUJITSU:2011-07-29 for FOTA DEL */
			dma_buffer->data.cfg0 = ((CFG0 & ~(7U << 6))			/* FUJITSU:2011-07-29 for FOTA ADD */
				& ~(1 << 4)) | ((((cwperpage >> 1)-1)) << 6);
			dma_buffer->data.cfg1 = chip->CFG1;
		} else {
			dma_buffer->data.cfg0 = ((MSM_NAND_CFG0_RAW &
...
msm_nand_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	unsigned page = 0;
	unsigned cwperpage;
/* FUJITSU:2011-06-12 change NAND bad block start */
    unsigned int bit_count;
    unsigned int bit_mask;
    unsigned int loop;
/* FUJITSU:2011-06-12 change NAND bad block end */
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	chip->CFG0 = (((mtd_writesize >> 9)-1) << 6) /* 4/8 cw/pg for 2/4k */
		...
		| ((wide_bus) ? (0 << 23) : (1 << 23));
	CFG0_A = chip->CFG0;	/* FUJITSU:2011-07-29 for FOTA ADD */
	...
	if (dual_nand_ctlr_present) {
		/* mtd->read_oob = msm_nand_read_oob_dualnandc; *//* FUJITSU:2011-06-25 for FOTA DEL */
