...
/* FUJITSU:2011-04-14 USB end */
struct msm_otg *the_msm_otg;
/* FUJITSU:2011-04-15 USB start */
extern void msm_hsusb_chg_set_status(int chg);
typedef void (*msm_batt_cbfunc)(void);
static msm_batt_cbfunc msm_batt_update_status_cb_otg = NULL;
/* FUJITSU:2011-04-15 USB end */
/* FUJITSU:2011-05-20 USB start */
#if 0	// USB_GB_OUT_EIF
static unsigned char no_change_int_flag = 0;
static int ReceivedCableDetectEvent = 0;
extern int hsusb_notifi_usb_ctrl_power(uint32_t ctrl);
/* FUJITSU:2011-05-20 USB end */
/* FUJITSU:2011-05-25 USB start */
static int is_host(void);
static int is_b_sess_vld(void);
static int is_host_a(void);
static int is_b_sess_vld_a(void);
static int is_host_a(void)
{
...
//static unsigned char is_b_sess_vld_a(void)
static int is_b_sess_vld_a(void)
{
	struct msm_otg *dev = the_msm_otg;
...
/* FUJITSU:2011-07-13 USB start */
	pr_info("[72kotg]msm_otg_set_power: Charging with %dmA current\n", charge);
...
static void msm_otg_start_peripheral(struct otg_transceiver *xceiv, int on)
{
	...
	if (!xceiv->gadget)
		return;
/* FUJITSU:2011-06-21 USB_DEBUG start */
	pr_info("[72kotg] msm_otg_start_peripheral on[%d]\n", on);
/* FUJITSU:2011-06-21 USB_DEBUG end */
	if (on) {
		if (pdata->setup_gpio)
...
static void msm_otg_start_host(struct otg_transceiver *xceiv, int on)
{
	...
	if (!xceiv->host)
		return;
/* FUJITSU:2011-06-21 USB_DEBUG start */
	pr_info("[72kotg] msm_otg_start_host on[%d]\n", on);
/* FUJITSU:2011-06-21 USB_DEBUG end */
	if (dev->start_host) {
...
#if 0	/* USB_GB Out 2011-05-06	*/
	if(system_rev >= 0x0c){
#endif	/* USB_GB Out 2011-05-06	*/
		msm_hsusb_chg_set_status(0);

		if(msm_batt_update_status_cb_otg)
			msm_batt_update_status_cb_otg();
		else
			printk("%s:msm_batt_update_status_cb_otg is NULL.\n", __func__);
#if 0	/* USB_GB Out 2011-05-06	*/
	}
#endif	/* USB_GB Out 2011-05-06	*/
...
/* FUJITSU:2011-07-13 USB end */
	...
	{
		if (system_rev < 0x0c){
			hsusb_notifi_usb_ctrl_power(0); /* USB POWER OFF */
			pr_info("%s: usb power off \n", __func__);
		}
	}
/* FUJITSU:2011-05-31 USB end */
...
/* FUJITSU:2011-07-13 USB_DEBUG start */
	if (!(sts & STS_PCI)){
		pc = readl(USB_PORTSC);
		intr = readl(USB_USBINTR);
		pr_info("[72kotg_int] STATE:(%s) OTGSC:(%08x) \n", state_string(state), otgsc);
		pr_info("[72kotg_int] STATUS:(%08x) PORTSC:(%08x) INTREG:(%08x)\n", sts, pc, intr);
	}
/* FUJITSU:2011-07-13 USB_DEBUG end */
...
/* FUJITSU:2011-04-20 USB start */
void msm_reg_battfunc_otg(msm_batt_cbfunc func)
{
        msm_batt_update_status_cb_otg = func;
        return;
}
EXPORT_SYMBOL(msm_reg_battfunc_otg);
/* FUJITSU:2011-04-20 USB end */
/* FUJITSU:2011-05-20 USB start */
#if 0	// USB_GB_OUT_EIF
void msm_otg_enable_irq(void)
...
out:
	...
	otgsc = readl(USB_OTGSC);
	printk(KERN_DEBUG "USB_DEBUG:%s ID->(%s) \n",__func__,(otgsc & OTGSC_ID)?"B":"A");
	if( !(otgsc & OTGSC_ID) )
	{
...
void msm_otg_disable_irq(void)
{
	...
	printk(KERN_DEBUG "USB_DEBUG:%s  \n",__func__); 
	if(no_change_int_flag == 0)
	{
		ReceivedCableDetectEvent=0;
		msm_otg_suspend(dev);
		hsusb_cable_det_notifi_disconnect();
		printk(KERN_DEBUG "USB_DEBUG:%s flag \n",__func__); 
	}
}
void msm_usb_phy_reg_init(void)
{
	...
	if (dev->hs_cclk)
	{
		printk(KERN_DEBUG "%s:USB_DEBUG:enable core clock!\n",__func__); 
		clk_enable(dev->hs_cclk);
	}
	clk_enable(dev->hs_clk);
	...
	if (dev->hs_cclk)
	{
		printk(KERN_DEBUG "USB_DEBUG:%s disable core clock!\n",__func__);
		clk_disable(dev->hs_cclk);
	}
