...
extern int hsusb_cable_det_notifi_initialize_complete(uint32_t drv);
extern int hsusb_cable_usb_vdd_off(void);
...
extern void serial_notification_detect_cable(int connect);
...
/* FUJITSU:2011-04-26 USB_GB_LISMO start */
	serial_notification_detect_cable(1);
/* FUJITSU:2011-04-26 USB_GB_LISMO end */
...
/* FUJITSU:2011-04-21 USB start */
		pr_info("%s: AC Charger detected [%d]\n",__func__,temp);
...
static void handle_setup(struct usb_info *ui)
{
	...
	int i;
	unsigned int	wDescriptorType = 0;
	unsigned char	stall;
	int				usb_req_get_descriptor_responce_table_count;
	
	static struct USB_REQ_GET_DESCRIPTOR_RESPONCE_TABLE {
		unsigned char	bDescriptorType;
		unsigned char	stall;
	} device_descriptor_stall_table[] = {	/* bDescriptorType			*/	/* stall 0=Not Stall 1=Stall								*/
											{USB_DT_DEVICE					,0x00	},	/* 01	Device Descriptor						*/
											{USB_DT_CONFIG					,0x00	},	/* 02	Configuration Descriptor				*/
											{USB_DT_STRING					,0x00	},	/* 03	String Descriptor						*/
											{USB_DT_INTERFACE				,0x00	},	/* 04	Interface Descriptor					*/
											{USB_DT_ENDPOINT				,0x00	},	/* 05	Endpoint Descriptor						*/
											{USB_DT_DEVICE_QUALIFIER		,0x00	},	/* 06	Device_Qualifier Descriptor				*/
											{USB_DT_OTHER_SPEED_CONFIG		,0x00	},	/* 07	Other_Speed_Configuration Descriptor	*/
											{USB_DT_INTERFACE_POWER			,0x01	},	/* 08	Interface Power Descriptor				*/
											{USB_DT_OTG						,0x01	},	/* 09	OTG										*/
											{USB_DT_DEBUG					,0x01	},	/* 10	DEBUG									*/
											{USB_DT_INTERFACE_ASSOCIATION	,0x01	},	/* 11	Interface_association					*/
											{USB_DT_SECURITY				,0x01	},
											{USB_DT_KEY						,0x01	},
											{USB_DT_ENCRYPTION_TYPE			,0x01	},	
											{USB_DT_BOS						,0x01	},	/* 15	BOS										*/
											{USB_DT_DEVICE_CAPABILITY		,0x01	},	/* 16	Device Capability						*/
											{USB_DT_WIRELESS_ENDPOINT_COMP	,0x01	},
											{USB_DT_WIRE_ADAPTER			,0x01	},
											{USB_DT_RPIPE					,0x01	},
											{USB_DT_SS_ENDPOINT_COMP		,0x01	},	/* 48	Superspeed_Usb_Endpoint_Companion		*/
											{0								,0x01	},	/* End of Table									*/
										  };
	...
	if (ctl.bRequest == USB_REQ_GET_DESCRIPTOR ) {
		...
		usb_req_get_descriptor_responce_table_count = (sizeof(device_descriptor_stall_table) / sizeof(device_descriptor_stall_table[0]));

		wDescriptorType = (ctl.wValue >>8 );
		stall = 0;
		for (i = 0 ; i < usb_req_get_descriptor_responce_table_count ; i++) {

			if (wDescriptorType == device_descriptor_stall_table[i].bDescriptorType) {
				/*pr_info("msm72k_udc.c:%s: USB_REQ_GET_DESCRIPTOR bDescriptorType_TABLE = 0x%x\n",__func__,device_descriptor_stall_table[i].bDescriptorType);	*/
				/*pr_info("msm72k_udc.c:%s: USB_REQ_GET_DESCRIPTOR stall = 0x%x\n",__func__,device_descriptor_stall_table[i].stall);	*/
				stall = device_descriptor_stall_table[i].stall;
				break;
			}
		}
...
static void usb_reset(struct usb_info *ui)
{
	...
	if (ui->driver) {
		dev_info(&ui->pdev->dev, "usb: notify offline\n");
		ui->driver->disconnect(&ui->gadget);
	}
	...
	for (;;) {
		...
		case USB_STATE_IDLE:
			if (flags & USB_FLAG_START) {
				...
				usb_reset(ui);
/* FUJITSU:2011-06-14 USB_GB start */
...
/* FUJITSU:2011-04-26 USB_GB_LISMO start */
				serial_notification_detect_cable(0);
/* FUJITSU:2011-04-26 USB_GB_LISMO end */
				switch_set_state(&ui->sdev, 0);
...
/* FUJITSU:2011-07-13 USB start */
pr_info("[gadget/msm72k_udc.c] %s() NON WALLCHARGER!!\n", __func__);
/* FUJITSU:2011-04-21 USB end */
				}
				else
				{
					...
					wake_unlock(&ui->wlock);
/* FUJITSU:2011-07-13 USB start */
pr_info("[gadget/msm72k_udc.c] %s() WALLCHARGER!!\n", __func__);
/* FUJITSU:2011-07-13 USB end */
				}
#endif
/* FUJITSU:2011-04-21 USB end */
...
/* FUJITSU:2011-06-22 USB start */
#if 0
	.wakeup		= msm72k_wakeup,
#else
	.wakeup		= NULL,
#endif
/* FUJITSU:2011-06-22 USB end */
...
/* FUJITSU:2011-04-21 USB start */
	...
	if (!msm72k_udc_is_host() || gadget_init_flag == 0) {
		usb_start(ui);
		gadget_init_flag = 1;
		pr_info("%s: usb_start gadget_init_flag[%d]\n",__func__,gadget_init_flag);
	}
#endif
/* FUJITSU:2011-04-21 USB end */
	return 0;
...
/* FUJITSU:2011-06-29 USB_GB_LISMO  start	*/
int is_usb_connected(void)
{
	struct usb_info *ui = the_usb_info;
	return is_usb_online(ui);
}
/* FUJITSU:2011-06-29 USB_GB_LISMO  end		*/
