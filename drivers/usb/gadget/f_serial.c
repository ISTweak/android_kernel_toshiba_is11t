...
extern int lismo_bind_config(struct usb_configuration *c, u8 port_num);
...
//	if (port_num == 0)
	...
	/* port_num 1:modem 2:nemea (0:lismo) */
	if (port_num == 1)
...
/* FUJITSU:2011-04-26 USB_GB_LISMO start */
int lismo_function_bind_config(struct usb_configuration *c)
{
	/* See if composite driver can allocate serial ports.
	 * Now allocate three ports for lismo, modem and nmea.
	 */
	int ret = gserial_setup(c->cdev->gadget, 3);
	if (ret)
		return ret;
	return lismo_bind_config(c, 0);
}
static struct android_usb_function lismo_function = {
	.name = "lismo",
	.bind_config = lismo_function_bind_config,
};
/* FUJITSU:2011-04-26 USB_GB_LISMO end */
...
{
/* FUJITSU:2011-04-26 USB_GB_LISMO start */
	android_register_function(&lismo_function);
/* FUJITSU:2011-04-26 USB_GB_LISMO end */
