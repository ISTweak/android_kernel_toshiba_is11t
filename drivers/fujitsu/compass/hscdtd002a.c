...
static int hscd_read_regs(uint8_t start_reg, uint8_t reg_data[], uint16_t n_regs)
{
	...
	for (;;) {
		...
		if (ret == -ETIMEDOUT) {
			compass_printk(KERN_ERR, "%s: %s I2C TimeOut error 0x%02X (%d)\n",
										HSCD_DEVNAME, __func__, start_reg, ret);
			goto Exit;
		}

		if (retry_count >= hscd->max_retry_count) {
			// When ..retrying.. frequency reaches the upper bound:error
			compass_printk(KERN_ERR, "%s: I2C retry over 0x%02X (%d)\n",
												HSCD_DEVNAME, start_reg, ret);
			goto Exit;
		}
		// It retries. 
		retry_count++;
...
Exit:
	if (error) {
		...
		compass_printk(KERN_ERR, "%s: %s read 0x%02X retry_count %d (%d)\n",
					HSCD_DEVNAME, __func__, start_reg, retry_count, ret);
	}
	return ret;
}
								 ...
								 uint8_t reg_data[], uint8_t n_regs)
{
	...
	for (;;) 
	{
		...
		if (read_count >= max_read_count) {
			compass_printk(KERN_ERR, "%s: %s retry over 0x%02X (%d)\n",
							HSCD_DEVNAME, __func__, start_reg, ret);
			goto RetryFailed;
		}
...
Exit:
	if (read_count > 0) {
		...
		hscd->error_count++;
		compass_printk(KERN_ERR, "%s: %s read 0x%02X retry_count %d (%d)\n",
					HSCD_DEVNAME, __func__, start_reg, read_count, ret);
	}
						   ...
						   const uint8_t reg_data[], uint16_t n_regs)
{
	...
	if (n_regs > HSCD_MAX_CONTIGUOUS_REGS) {
		ret = -EINVAL;
		compass_printk(KERN_ERR, "%s: %s Too many registers (%u > %u).\n",
					HSCD_DEVNAME, __func__, n_regs, HSCD_MAX_CONTIGUOUS_REGS);
		goto Error;
	}
...
Error:
		hscd->error_count++;
		hscd->retry_count += retry_count;
		compass_printk(KERN_ERR, "%s: %s write 0x%02X retry_count %d (%d)\n",
					HSCD_DEVNAME, __func__, start_reg, retry_count, ret);
	}
	debug("%s() -> %d\n", __func__, ret);
	return ret;
								  ...
								  uint16_t n_regs, uint8_t verify_mask)
{
	...
	if ((verify_mask & 0xFF) == 0) {
		ret = -EINVAL;
		compass_printk(KERN_ERR, "%s: %s check mask injustice (0x%02X).\n",
						HSCD_DEVNAME, __func__, verify_mask);
		goto Error;
	}
	...
	for (;;) {
		...
		if (ret < 0) {
			compass_printk(KERN_ERR, "%s: %s Can't Write 0x%02X.\n",
									HSCD_DEVNAME, __func__, start_reg);
			goto Error;
		}

		// The register is read buffer[].
		if ((ret = hscd_read_regs(start_reg, buffer, n_regs)) < 0) {
			compass_printk(KERN_ERR, "%s: %s Can't Read 0x%02X.\n",
									HSCD_DEVNAME, __func__, start_reg);
			goto Error;
		}
		...
		if (retry_count >= hscd->max_retry_count) {
			compass_printk(KERN_ERR, "%s: Reg write-verify retry over 0x%02X (%d)\n",
												HSCD_DEVNAME, start_reg, ret);
			ret = -EIO;
			goto Error;
		}
	...
	if (mode == HSCD_MODE_STANBY_V2 || mode == HSCD_MODE_STANBY_V3) {
		...
		if (ret < 0) {
			compass_printk(KERN_ERR, 
				"%s: %s mode_stanby Can't Write HSCD_CNTL1.\n",
											HSCD_DEVNAME, __func__);
			goto Exit;
		}
		//CNTL3 clear
		...
		if (ret < 0) {
			compass_printk(KERN_ERR, 
				"%s: %s mode_stanby Can't Write HSCD_CNTL3.\n",
											HSCD_DEVNAME, __func__);
			goto Exit;
		}
		//CNTL2 clear
		...
		if (ret < 0) {
			compass_printk(KERN_ERR, 
				"%s: %s mode_stanby Can't Write HSCD_CNTL2.\n",
											HSCD_DEVNAME, __func__);
			goto Exit;
		}
		...
		if (hscd->measurement.measurement_range == HSCD_MEASUREMENT_RANGE_12) {
			...
			//error
			ret = -EINVAL;
			compass_printk(KERN_ERR, "%s: %s measurement range error.%d\n",
				HSCD_DEVNAME, __func__, hscd->measurement.measurement_range);
			goto Exit;
		}
		...
		if (ret < 0) {
			compass_printk(KERN_ERR, 
				"%s: %s mode_forcestate Can't Write HSCD_CNTL1.\n",
												HSCD_DEVNAME, __func__);
			goto Exit;
		}
		// CNTL2 is DRDY ON
		...
		if (ret < 0) {
			compass_printk(KERN_ERR, 
				"%s: %s mode_forcestate Can't Write HSCD_CNTL2.\n",
												HSCD_DEVNAME, __func__);
			goto Exit;
		}
		...
		if (ret < 0) {
			compass_printk(KERN_ERR, 
				"%s: %s mode_forcestate Can't Write HSCD_CNTL3.\n",
												HSCD_DEVNAME, __func__);
			goto Exit;
		}
		...
		//error
		ret = -EINVAL;
		compass_printk(KERN_ERR, "%s: %s mode error.(0x%02X)\n",
									HSCD_DEVNAME, __func__, mode);
		goto Exit;
	}
...
--------------------------------------------------------------------*/
#define hscd_stanby()      hscd_setmode(HSCD_MODE_STANBY_V3)
...
static int hscd_read_reg_all(void)
{
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_TEMP.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_XLSB.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_STAT.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	// read CNTL1
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_CNTL1.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	// read CNTL2
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_CNTL2.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	// read CNTL3
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_CNTL3.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	// read CNTL4
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_CNTL4_V3.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	// read PRET
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_PRET.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	...
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_XOFFLSB.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	// MSB/LSB of buffer is replaced
...
static int hscd_set_sensor_offset(const hscd_xyz_t * off)
{
	...
	if (ret == OK) {
		...
		compass_printk(KERN_ERR, "%s: %s  Can't Write HSCD_XOFFLSB.\n",
											HSCD_DEVNAME, __func__);
	}
...
static int hscd_set_sensor_ampgain(const uint8_t *amp)
{
	...
	if (ret == OK) {
		...
		compass_printk(KERN_ERR, "%s: %s  Can't Write HSCD_CNTL4_V3.\n",
											HSCD_DEVNAME, __func__);
	}
	debug("%s(amp:%u) -> %d\n", __func__, *amp, ret);
...
static int hscd_offsetcalib(void)
{
	...
	ret = hscd_write_verify_regs(HSCD_CNTL1, &mode, 1, 0x80);
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Write HSCD_CNTL1.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	...
	ret = hscd_write_regs(HSCD_CNTL3, &cntl3, 1);
	if (ret < 0) {
		compass_printk(KERN_ERR, "%s: %s  Can't Write HSCD_CNTL3.\n",
											HSCD_DEVNAME, __func__);
		goto Exit;
	}
	...
	for (;;) {
		...
		if (retry_count >= hscd->max_retry_count) {
			// when retries reach upper limit:error
			compass_printk(KERN_ERR, "%s retry over: Unable to OFFSET CALIBRATION.\n", __func__);
			//ret = -EIO;
			break;
		}
		// retries
...
static int hscd_clear_interrupt(void)
{
	...
	if (likely(GPIO_ISACTIVE())) {
		...
		for (;;) {
			...
			if (retry_count >= hscd->max_retry_count) {
				// when retries reach upper limit:error
				compass_printk(KERN_ERR, "%s retry over: Unable to clear GPIO97.\n", __func__);
				break;
			}
			// retries
			retry_count++;
...
static int hscd_measure_end(void)
{
	...
	if (ret >= 0) {
		...
		debug("%s():DATA:(0x%04X 0x%04X 0x%04X)\n",
				__func__, hscd->reg.data.x,hscd->reg.data.y, hscd->reg.data.z);
...
static int hscd_ioctl_measure(compass_data_t __user * measurement)
{
	...
	if ((ret = down_interruptible(&hscd_sem)) >= 0) {
		...
		if (ret != 0) {
			compass_printk(KERN_ERR, 
				"%s: %s Can't copy from user area.%d.\n",
		...
		for (;;) {
			...
			if (ret < 0)  {
				compass_printk(KERN_ERR, 
					"%s: %s wait_event_interruptible_timeout ret=%d.\n",
											HSCD_DEVNAME, __func__, ret);
				goto Exit;
			}
			...
			if (measurement_count > HSCD_MAX_MEASUREMENT_COUNT) {
				compass_printk(KERN_ERR, 
					"%s %s: measurement wait event error.\n", __func__, HSCD_DEVNAME);
				ret = -EBUSY;
				goto Exit;
			}
		}
		...
		ret = copy_to_user(measurement, &hscd->measurement,
										sizeof(*measurement));
		if (ret != 0) {
			compass_printk(KERN_ERR, "%s: %s Can't copy to user area.%d.\n",
							HSCD_DEVNAME, __func__, ret);
			ret = -EFAULT;
		}
	...
	if ((ret = down_interruptible(&hscd_sem)) >= 0) {
		...
		if (hscd->use_count == 0) {
			...
			ret = request_irq(HSCD_IRQ(), hscd_isr, IRQ_TYPE_EDGE_RISING,
							HSCD_DEVNAME, &hscd);
			if (unlikely(ret < 0)) {
				compass_printk(KERN_ERR, "%s: Can't use IRQ%u. ret=%d\n",
												HSCD_DEVNAME, HSCD_IRQ(), ret);
				goto Exit2;
			}
  			disable_irq(HSCD_IRQ());
		}
...
static int hscd_ioctl_setregs(const hscd_registers_t __user * regs)
{
	...
	if ((ret = down_interruptible(&hscd_sem)) >= 0) {
		...
		// src <- *regs
		ret = copy_from_user(&src, regs, sizeof(src));
		if (ret != 0) {
			compass_printk(KERN_ERR, "%s: %s Can't copy from user area.%d\n",
													HSCD_DEVNAME, __func__,ret);
			ret = -EFAULT;
			goto Exit;
		}
		if (src.mask & HSCD_REGMASK_OFFSET) {
			...
			if ((ret = hscd_set_sensor_offset(&src.off)) < 0) {
				compass_printk(KERN_ERR, "%s: %s Can't set_offset.%d\n",
												HSCD_DEVNAME, __func__,ret);
				goto Exit;
			}
		...
		if (src.mask & HSCD_REGMASK_AMPGAIN) {
			...
			if ((ret = hscd_set_sensor_ampgain(&src.amp)) < 0) {
				compass_printk(KERN_ERR, "%s: %s Can't set_ampgain.%d\n",
												HSCD_DEVNAME, __func__,ret);
				goto Exit;
			}
...
static int hscd_ioctl_getregs(hscd_registers_t __user * regs)
{
	...
	if ((ret = down_interruptible(&hscd_sem)) >= 0) {
		...
		// mask <- regs->mask
		ret = copy_from_user(&mask, &regs->mask, sizeof(mask));
		if (ret != 0) {
			compass_printk(KERN_ERR, "%s: %s Can't copy from user area.%d\n",
												HSCD_DEVNAME, __func__, ret);
			ret = -EFAULT;
		...
		if (mask & HSCD_REGMASK_DATA) {
			...
			ret = copy_to_user(&regs->temp, buffer, nRegs);
			if (ret != 0) {
				compass_printk(KERN_ERR, 
					"%s: %s Can't copy user temp area.%d\n",
											HSCD_DEVNAME, __func__, ret);
				goto CopyFailed;
			}
			...
			hscd->reg.data.y = buffer[2] + (buffer[3] << 8);
			hscd->reg.data.z = buffer[4] + (buffer[5] << 8);
			ret = copy_to_user(&regs->data, &hscd->reg.data, nRegs);
			if (ret != 0) {
				compass_printk(KERN_ERR, 
					"%s: %s Can't copy user data area.%d\n",
											HSCD_DEVNAME, __func__, ret);
				goto CopyFailed;
			}
		...
		if (mask & HSCD_REGMASK_OFFSET) {
			...
			ret = copy_to_user(&regs->off, &hscd->reg.off, nRegs);
			if (ret != 0) {
				compass_printk(KERN_ERR, 
					"%s: %s Can't copy user off area.%d\n",
											HSCD_DEVNAME, __func__, ret);
				goto CopyFailed;
			}
			// set flag of hscd->reg.mask
		...
		if (mask & HSCD_REGMASK_AMPGAIN) {
			...
			// To store the contents read
			ret = copy_to_user(&regs->amp, buffer, nRegs);
			if (ret != 0) {
				compass_printk(KERN_ERR, 
					"%s: %s Can't copy user amp area.%d\n",
											HSCD_DEVNAME, __func__, ret);
				goto CopyFailed;
			}
			// set flag of hscd->reg.mask
		...
		if (gpio_get_value(40)){
			hscd->reg.case_open = 0;
		} else {
			hscd->reg.case_open = 1;
		}
		ret = copy_to_user(&regs->case_open, &hscd->reg.case_open, 1);
		if (ret != 0) {
			compass_printk(KERN_ERR, "%s: %s Can't copy user case area.%d\n",
												HSCD_DEVNAME, __func__, ret);
			goto CopyFailed;
		}
...
static int hscd_ioctl_offcal(hscd_registers_t __user * regs)
{
	...
	if ((ret = down_interruptible(&hscd_sem)) >= 0) {
		...
		if ((ret = hscd_offsetcalib()) > 0) {
			...
			if (ret != 0) {
				compass_printk(KERN_ERR, 
					"%s: %s Can't copy to user off area.%d\n",
									HSCD_DEVNAME, __func__, ret);
				goto CopyFailed;
			}
		}
...
static int hscd_ioctl_selftest(selftest_data_t __user * selfdata)
{
	...
	if ((ret = down_interruptible(&hscd_sem)) >= 0) {
		...
		// mode <- *selfdata->mode
		ret = copy_from_user(&mode, &selfdata->test_mode, sizeof(mode));
		if (ret != 0) {
			compass_printk(KERN_ERR, "%s: %s Can't copy from user area.%d\n",
												HSCD_DEVNAME, __func__, ret);
			ret = -EFAULT;
		...
		if (mode == MODE_A) {
			...
			if (ret < 0) {
				compass_printk(KERN_ERR, "%s: %s  Can't Read1 HSCD_STBA.\n",
													HSCD_DEVNAME, __func__);
				goto Exit;
			}
			...
			if (check_data != HSCD_SELFTEST_DEF) {
				compass_printk(KERN_ERR, 
					"%s: %s STBA Initial value err.(0x%02X)\n",
							HSCD_DEVNAME, __func__, check_data);
				ret = -EFAULT;
				goto Exit;
			}
			...
			if (ret < 0) {
				compass_printk(KERN_ERR, "%s: %s  Can't Read2 HSCD_STBA.\n",
													HSCD_DEVNAME, __func__);
				goto Exit;
			}
			memcpy(&check_data, buffer, sizeof(check_data));
			...
			ret = hscd_read_verify_regs(HSCD_STBA, buffer,1);
			if (ret < 0) {
				compass_printk(KERN_ERR, "%s: %s  Can't Read3 HSCD_STBA.\n",
													HSCD_DEVNAME, __func__);
				goto Exit;
			}
			memcpy(&check_data, buffer, sizeof(check_data));
			...
			if (ret < 0) {
				compass_printk(KERN_ERR, "%s: %s  Can't Read1 HSCD_STBB.\n",
													HSCD_DEVNAME, __func__);
				goto Exit;
			}
			...
			if (check_data != HSCD_SELFTEST_DEF) {
				compass_printk(KERN_ERR,
					"%s: %s STBB Initial value err.(0x%02X)\n",
							HSCD_DEVNAME, __func__, check_data);
				ret = -EFAULT;
				goto Exit;
			}
			...
			if (ret < 0) {
				compass_printk(KERN_ERR, "%s: %s  Can't Read2 HSCD_STBB.\n",
													HSCD_DEVNAME, __func__);
				goto Exit;
			}
			
			//error_code get
			ret = copy_to_user(&selfdata->err_code,
								buffer, sizeof(selfdata->err_code)) ;
			if (ret != 0) {
				compass_printk(KERN_ERR, "%s: %s Can't copy user area.%d\n",
												HSCD_DEVNAME, __func__, ret);
				ret = -EFAULT;
				goto Exit;
			}
			...
			ret = hscd_read_verify_regs(HSCD_STBB, buffer,1);
			if (ret < 0) {
				compass_printk(KERN_ERR, "%s: %s  Can't Read3 HSCD_STBB.\n",
													HSCD_DEVNAME, __func__);
				goto Exit;
			}
			memcpy(&check_data, buffer, sizeof(check_data));
			...
			ret = -EFAULT;
			compass_printk(KERN_ERR, "%s: %s mode error.%d\n",
										HSCD_DEVNAME, __func__, mode);
		}
...
int hscd_ioctl_measure_temp(compass_data_t __user * measurement) {
	...
	for(;;) 
	{
		...
		ret = hscd_read_verify_regs(HSCD_STAT, &buffer, sizeof(hscd->reg.stat));
		if (ret < 0) {
			compass_printk(KERN_ERR, "%s: %s  Can't Read HSCD_STAT.\n",
											HSCD_DEVNAME, __func__);
			goto Exit;
		...
		if (retry_count > 3) {
			compass_printk(KERN_ERR,"%s: %s temp data get error.(0x%02X)\n",
											HSCD_DEVNAME, __func__, buffer);
			ret = -EIO;
			goto Exit;
		}
	...
	ret = copy_to_user(measurement, &hscd->measurement, sizeof(*measurement));
	if (ret != 0) {
		compass_printk(KERN_ERR," %s: %s Can't copy to user area.%d\n",
										HSCD_DEVNAME, __func__, ret);
		ret = -EFAULT;
