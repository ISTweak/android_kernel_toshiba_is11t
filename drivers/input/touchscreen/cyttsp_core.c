...
/* FUJITSU:2011/03/03 mod start */
#define FJ_BOOTLOADER_KEYS
#ifdef FJ_BOOTLOADER_KEYS
...
/* FUJITSU:2011/07/08 mod start */
#define CY_TOP_YAXIS_THD            10
#define CY_BOTTOM_YAXIS_THD         789
#define CY_ZAXIS_THD                45
#define CY_I2C_NACK_WAIT_VALUE      1
#define CY_OVERALL_ZAXIS_THD        50
/* FUJITSU:2011/07/08 mod end */
...
static int ttsp_write_block_data(struct cyttsp *ts, u8 command, u8 length, void *buf);
static int ttsp_read_block_data(struct cyttsp *ts, u8 command, u8 length, void *buf);
...
/* FUJITSU:2011/04/27 add start */
static int cyttsp_ioctl( struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
	{
	int ret = 0;
	...
	switch(cmd){
		case IOCTL_SET_POWERMODE:
			...
			switch(mode){
				...
				case HARDRESET:
					...
					ret = ts->platform_data->wakeup_xres();
					if (ret < 0)
						{
						printk(KERN_ERR "Error, XReset wakeup failed!\n");
						}
		...
		case IOCTL_GET_FIRMVERSION:
			...
			if(ret < 0)
				{
				printk(KERN_ERR "FIRMVERSION set sys %dn",ret);
				}
			...
			if(ret < 0)
				{
				printk(KERN_ERR "FIRMVERSION set ope %d",ret);
				}
		...
		case IOCTL_DO_CALIBRATION:
				...
				if(ret < 0)
					{
					printk(KERN_ERR "CALIBRATION set sys %d",ret);
					}

				/* write data 0x23 reg 2 */
				setcmd = 0x23;
				ret = ttsp_write_block_data(ts, CY_REG_MFG_CMD, sizeof(setcmd), &setcmd);
				if (ret < 0)
					{
					printk(KERN_ERR "ioctl write data 0x23 reg 2 err:%d\n",ret);
					}

				/* write data 0x00 reg 3 */
				setcmd = 0x00;
				ret = ttsp_write_block_data(ts, CY_REG_CID_0, sizeof(setcmd), &setcmd);
				if (ret < 0)
					{
					printk(KERN_ERR "ioctl write data 0x00 reg 3 err:%d\n",ret);
					}

				/* write data 0x00 reg 4 */
				setcmd = 0x00;
				ret = ttsp_write_block_data(ts, CY_REG_CID_1, sizeof(setcmd), &setcmd);
				if (ret < 0)
					{
					printk(KERN_ERR "ioctl write data 0x00 reg 4 err:%d\n",ret);
					}
				...
				if (ret < 0)
					{
					printk(KERN_ERR "ioctl write data 0x20 reg 2 err:%d\n",ret);
					}
				...
				do {
					...
					mdelay(2000);
					count ++;
					ret = ttsp_read_block_data(ts, CY_REG_MFG_STAT,sizeof(rdata), &rdata);
				...
				if (ret < 0)
					{
					printk(KERN_ERR "ioctl write data 0x22 reg 2 err:%d\n",ret);
					}
				...
				if(ret < 0)
					{
					printk(KERN_ERR "CALIBRATION set ope %d",ret);
					}
			break;
...
static int cyttsp_open( struct inode *inode, struct file *file )
	{
...
static int cyttsp_close(struct inode *inode, struct file *file)
	{
	printk( KERN_INFO "%s :[IN]\n", __func__ );
    return 0;
	}
...
static struct file_operations cyttsp_fops = {
    .owner   = THIS_MODULE,
	.release = cyttsp_close,
    .open    = cyttsp_open,
    .ioctl   = cyttsp_ioctl,
	};

/* FUJITSU:2011/04/27 add end */
/* FUJITSU:2011/06/09 add start */
static int cyttsp_recovery_bl(struct cyttsp *ts)
{
	int retval = 0;
	...
	/* XReset wakeup */
	retval = ts->platform_data->wakeup_xres();
	if (retval < 0) {
		printk(KERN_ERR "%s: Error, XReset wakeup failed!\n", __func__);
	}
...
/* FUJITSU:2011/06/09 add end */
static int ttsp_read_block_data(struct cyttsp *ts, u8 command,
	u8 length, void *buf)
{
	int rc;
	int tries;
    /* FUJITSU:2011/07/07 add start */
	u32 i2c_param = 0x00;
    /* FUJITSU:2011/07/07 add end */
	...
	for (tries = 0, rc = -1; tries < CY_NUM_RETRY && (rc < 0); tries++) {
        ...
        /* FUJITSU:2011/07/07 mod start */
		if (rc) {
			i2c_param = ~rc + 0x01;
			if(tries == 0 && (i2c_param & (1U << 3))) {
//				printk("%s: I2C Read NACK Err Retry \n", __func__);
				mdelay(CY_I2C_NACK_WAIT_VALUE);
			} else {
//				printk("%s: I2C Read Err Retry \n", __func__);
				msleep(CY_DELAY_DFLT);
			}
		}
        /* FUJITSU:2011/07/07 mod end */
	}
...
static void handle_multi_touch(struct cyttsp_track_data *t, struct cyttsp *ts)
{
...
#if 0   /* FUJITSU:2011/07/07 mod start */
		input_report_abs(ts->input, ABS_MT_TRACKING_ID, (id - 1));
#endif  /* FUJITSU:2011/07/07 mod end */
	...
	for (id = 0; id < CY_NUM_MT_TCH_ID; id++) {
        ...
        if(ts->act_trk[trk_id] == CY_NTCH) {
            if(((t->cur_mt_pos[id][CY_YPOS] < CY_TOP_YAXIS_THD) && (t->cur_mt_z[id] < CY_ZAXIS_THD)) ||
                ((CY_BOTTOM_YAXIS_THD <  t->cur_mt_pos[id][CY_YPOS]) && (t->cur_mt_z[id] < CY_ZAXIS_THD))) {
                t->cur_trk[trk_id] = CY_NTCH;
                continue;
            }
            /* FUJITSU:2011/07/08 add start */
            if(t->cur_mt_z[id] < CY_OVERALL_ZAXIS_THD) {
                t->cur_trk[trk_id] = CY_NTCH;
                continue;
            }
            /* FUJITSU:2011/07/08 add end */
        }
        /* FUJITSU:2011/07/07 add end */
		...
		input_report_abs(ts->input, ABS_MT_TRACKING_ID,
						(t->cur_mt_tch[id] - 1));
#endif  /* FUJITSU:2011/07/07 mod start */
...
static void cyttsp_xy_worker(struct cyttsp *ts)
{
...
#if 1   /* FUJITSU:2011/06/09 Adjustment start */
	if (CY_MODE_ERROR(ts->platform_data->power_state,
		...
		printk(KERN_ERR "%s: Error, bad mode ps=%d hm=%02X tm=%02X\n",
			...
			xy_data.hst_mode, xy_data.tt_mode);
		retval = cyttsp_recovery_bl(ts);
		if (retval < 0)
			printk(KERN_ERR "%s: Error, recovery bl fail\n", __func__);
		goto exit_xy_worker;
	}
#endif   /* FUJITSU:2011/06/09 Adjustment end */
	...
	case 4:
        ...
        /* FUJITSU:2011/03/10 add start */
        if ((xy_data.x4 > ts->platform_data->maxx) || 
            (xy_data.y4 > ts->platform_data->maxy)){
            DBG(printk(KERN_INFO"xy4 over Area \n");)
            goto exit_xy_worker;
        }
        /* FUJITSU:2011/03/10 add end */
		if (ts->platform_data->use_trk_id) {
	...
	case 3:
        ...
        /* FUJITSU:2011/03/10 add start */
        if ((xy_data.x3 > ts->platform_data->maxx) || 
            (xy_data.y3 > ts->platform_data->maxy)){
            DBG(printk(KERN_INFO"xy3 over Area \n");)
            goto exit_xy_worker;
        }
        /* FUJITSU:2011/03/10 add end */
		if (ts->platform_data->use_trk_id) {
	...
	case 2:
        ...
        /* FUJITSU:2011/03/10 add start */
        if ((xy_data.x2 > ts->platform_data->maxx) || 
            (xy_data.y2 > ts->platform_data->maxy)){
            DBG(printk(KERN_INFO"xy2 over Area \n");)
            goto exit_xy_worker;
        }
        /* FUJITSU:2011/03/10 add end */
		if (ts->platform_data->use_trk_id) {
	...
	case 1:
        ...
        /* FUJITSU:2011/03/10 add start */
        if ((xy_data.x1 > ts->platform_data->maxx) || 
            (xy_data.y1 > ts->platform_data->maxy) ){
            DBG(printk(KERN_INFO"xy1 over Area \n");)
            goto exit_xy_worker;
        }
        /* FUJITSU:2011/03/10 add end */
		if (ts->platform_data->use_trk_id) {
	...
	/* FUJITSU:2011/06/28 mod start */
	memcpy(ts->act_trk, trc.cur_trk, sizeof(ts->act_trk));
	/* FUJITSU:2011/06/28 mod end */
...
static int cyttsp_exit_bl_mode(struct cyttsp *ts)
{
	 ...
	 */
#if 0  /* FUJITSU:2011/05/31 mod start */
	cyttsp_load_bl_regs(ts);
	if (!GET_BOOTLOADERMODE(ts->bl_data.bl_status))
		return 0;
#endif /* FUJITSU:2011/05/31 mod end */
...
static int cyttsp_set_sysinfo_mode(struct cyttsp *ts)
{
	...
	/* FUJITSU:2011/05/31 add start */
	msleep(20);
	/* FUJITSU:2011/05/31 add end */
...
exit_sysinfo_mode:
	...
	printk(KERN_INFO"%s:SI2:act_intrvl=0x%02X lp_intrvl=0x%02X tts_ver=%02X%02X app_id=%02X%02X "
		"app_ver=%02X%02X c_id=%02X%02X%02X "
		"u_id=%02X%02X%02X%02X%02X%02X%02X%02X\n",
		__func__, ts->sysinfo_data.act_intrvl,ts->sysinfo_data.lp_intrvl,
...
{
	int retval = 0;
	/* FUJITSU:2011/05/30 mod start */
	u8 cmd;
	u8 act_cmd = 0x11;
	u8 set_cmd = 0x00;
	/* FUJITSU:2011/07/19 add start */
	u8 self_fth_cmd = 3;
	/* FUJITSU:2011/07/19 add start */


	retval = ttsp_write_block_data(ts, CY_REG_ACT_INTRVL, sizeof(act_cmd), &act_cmd);
	retval = ttsp_write_block_data(ts, 0x23, sizeof(set_cmd), &set_cmd);
	/* FUJITSU:2011/07/19 add start */
	retval = ttsp_write_block_data(ts, 0x36, sizeof(self_fth_cmd), &self_fth_cmd);
	/* FUJITSU:2011/07/19 add start */

	cmd = ts->sysinfo_data.hst_mode & CY_HNDSHK_BIT ?
	...
	msleep(20);
	/* FUJITSU:2011/05/30 mod end */

/* FUJITSU:2011/04/20 mod start */
#if 0
...
//	struct cyttsp_xydata xy_data;
	...
	/* FUJITSU:2011/05/30 add start */
	msleep(20);
	/* FUJITSU:2011/05/30 add end */

#if 0  /* FUJITSU:2011/05/30 mod start */
	tries = 0;
	gest_default =
	...
	DBG2(printk(KERN_INFO "%s: read tries=%d\n", __func__, tries);)
#endif /* FUJITSU:2011/05/30 mod end */
...
/* FUJITSU:2011/04/27 add start */
	...
	result = alloc_chrdev_region(&dev, 0, 1, "cyttsp");
	...
	result = cdev_add ( &(ts->cyttsp_cdev), devno, 1 );
