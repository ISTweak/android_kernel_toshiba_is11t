...
/* FUJITSU:2011-04-14 Add for OPEN_DET start */
#include <linux/gpio.h>
#include <linux/wakelock.h>
/* FUJITSU:2011-04-14 Add for OPEN_DET end */
...
/* FUJITSU:2011-04-14 Add for OPEN_DET start */
#define THIS_FILE	"pmic8058_keypad"
#define PACOKBD_DEBUG	(1)
#define DBG_FUNC	(0x00000001UL)
#define DBG_LOG		(0x00000002UL)
static unsigned long g_debug_control = DBG_FUNC | DBG_LOG;
#if defined(PACOKBD_DEBUG) == 1
#define FUNCENTER(fmt, args...) \
    do { if (g_debug_control & DBG_FUNC) { \
...
#define PACODBG(fmt, args...) \
    do { if (g_debug_control & DBG_LOG) { \
        printk(KERN_INFO THIS_FILE "@%d %s() " fmt "\n", \
            __LINE__, __FUNCTION__, args); \
    } } while (0)
#else
...
/* FUJITSU:2011-04-14 Add for OPEN_DET start */
	int detect_irq;
	int detect_irq_registered;
	int sw_lid;
	int provisional_tmode;
	atomic_t sample_cnt;
...
/* FUJITSU:2011-04-14 Add for OPEN_DET start */
int
pmic8058_kp_request_irq(unsigned int irq, irq_handler_t handler,
	unsigned long flags, const char * name, void * dev)
{
	return request_irq(irq, handler, flags, name, dev);
}
void pmic8058_kp_free_irq(unsigned int irq, void * dev)
{
	free_irq(irq, dev);
}
void pmic8058_kp_enable_irq(unsigned int irq)
{
	enable_irq(irq);
}
void pmic8058_kp_disable_irq(unsigned int irq)
{
	disable_irq(irq);
}
void pmic8058_kp_disable_irq_nosync(unsigned int irq)
{
	disable_irq_nosync(irq);
}
int pmic8058_kp_enable_irq_wake(unsigned int irq)
{
	return enable_irq_wake(irq);
}
int pmic8058_kp_disable_irq_wake(unsigned int irq)
{
	return disable_irq_wake(irq);
}
struct workqueue_struct * pmic8058_kp_create_workqueue(const char * name)
{
	return create_singlethread_workqueue(name);
}
void pmic8058_kp_destroy_workqueue(struct workqueue_struct * wq)
{
	if (wq)
		destroy_workqueue(wq);
}
void pmic8058_kp_init_delayed_work(struct delayed_work * w, work_func_t fn)
{
	INIT_DELAYED_WORK(w, fn);
}
int
pmic8058_kp_queue_delayed_work(
	struct workqueue_struct * wq, struct delayed_work * w, unsigned long delay)
{
	return queue_delayed_work(wq, w, delay);
}
int pmic8058_kp_cancel_delayed_work(struct delayed_work * w)
{
	return cancel_delayed_work(w);
}
void pmic8058_kp_wake_lock_init(struct wake_lock * lock, int type, const char * name)
{
	wake_lock_init(lock, type, name);
}
void pmic8058_kp_wake_lock(struct wake_lock * lock)
{
	wake_lock(lock);
}
void pmic8058_kp_wake_unlock(struct wake_lock * lock)
{
	wake_unlock(lock);
}
static ssize_t
pmic8058_kp_sw_lid_show(struct device * dev,
	struct device_attribute * attr, char * buf)
{
	struct pmic8058_kp * kp = dev_get_drvdata(dev);
	return snprintf(buf, 16, "%u\n", kp->sw_lid);
}
static DEVICE_ATTR(sw_lid, 0444, pmic8058_kp_sw_lid_show, NULL);
static ssize_t
pmic8058_kp_slide_count_show(struct device * dev,
	struct device_attribute * attr, char * buf)
{
	struct pmic8058_kp * kp = dev_get_drvdata(dev);
	return snprintf(buf, 16, "%u\n", kp->slide_count);
}
static DEVICE_ATTR(slide_count, 0444, pmic8058_kp_slide_count_show, NULL);
static int
pmic8058_kp_get_open_det(struct pmic8058_kp * kp)
{
	int gpio_value = 0;

	FUNCENTER("kq=%p", kp);

	gpio_value = gpio_get_value(40);

	FUNCLEAVE("gpio_value=%s", gpio_value ? "high" : "low");

	return gpio_value;
}
static irqreturn_t
pmic8058_kp_open_det_interrupt(int irq, void * dev_id)
{
	struct pmic8058_kp * kp = dev_id;

	FUNCENTER("irq=%d,dev_id=%p", irq, dev_id);

	if (!kp->resume_done) {
		FUNCLEAVE("%s", "IRQ_HANDLED");
		return IRQ_HANDLED;
	}

	kp->provisional_tmode = pmic8058_kp_get_open_det(kp);
	if (kp->sw_lid != kp->provisional_tmode) {
		pmic8058_kp_wake_lock(&kp->wakelock);
		pmic8058_kp_disable_irq_nosync(kp->detect_irq);
		atomic_set(&kp->sample_cnt, 3);
		pmic8058_kp_queue_delayed_work(kp->wq, &kp->detect_work, msecs_to_jiffies(30));
	}
	FUNCLEAVE("%s", "IRQ_HANDLED");
	return IRQ_HANDLED;
}
static void
pmic8058_kp_detect_work_fn(struct work_struct * w)
{
	struct pmic8058_kp * kp = container_of(w, struct pmic8058_kp, detect_work.work);
	unsigned int tmode = 0;
	FUNCENTER("w=%p", w);

	tmode = pmic8058_kp_get_open_det(kp);
	if (kp->provisional_tmode != tmode) {
		atomic_set(&kp->sample_cnt, 3);
		kp->provisional_tmode = tmode;
	}

	PACODBG("kp->sample_cnt.counter=%d", kp->sample_cnt.counter);

	if (!atomic_dec_and_test(&kp->sample_cnt))
		pmic8058_kp_queue_delayed_work(kp->wq, &kp->detect_work, msecs_to_jiffies(30));
	else {
		kp->sw_lid = tmode;
		input_report_switch(kp->input, SW_LID, kp->sw_lid);
		input_sync(kp->input);
		pmic8058_kp_wake_unlock(&kp->wakelock);
		if (kp->detect_irq_registered) {
			kp->slide_count++;
			kp->resume_done = 1;
			pmic8058_kp_enable_irq(kp->detect_irq);
		} else {
			kp->detect_irq_registered = !pmic8058_kp_request_irq(
				kp->detect_irq, pmic8058_kp_open_det_interrupt,
				IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_DISABLED,
				"open_det", kp) ? 1 : 0;

			pmic8058_kp_read_u8(kp, &kp->ctrl_reg, KEYP_CTRL);

			if (device_create_file(kp->dev, &dev_attr_disable_kp))
				dev_err(kp->dev, "can't create dev_attr_disable_kp\n");

			kp->class_dev = device_create(keyboard_class, kp->dev, 0, kp, "%s", kp->input->name);
			if (IS_ERR(kp->class_dev))
				dev_err(kp->dev, "can't create device\n");

			if (device_create_file(kp->class_dev, &dev_attr_sw_lid))
				dev_err(kp->dev, "can't create dev_attr_sw_lid\n");

			if (device_create_file(kp->class_dev, &dev_attr_slide_count))
				dev_err(kp->dev, "can't create dev_attr_slide_count\n");

			device_init_wakeup(kp->dev, kp->pdata->wakeup);
		}
		if (kp->sw_lid) {
			if (kp->ctrl_reg & KEYP_CTRL_KEYP_EN) {
				pmic8058_kp_disable_irq(kp->key_sense_irq);
				pmic8058_kp_disable_irq(kp->key_stuck_irq);
				kp->ctrl_reg &= ~KEYP_CTRL_KEYP_EN;
				pmic8058_kp_write_u8(kp, kp->ctrl_reg, KEYP_CTRL);
			}
		} else {
			if (!(kp->ctrl_reg & KEYP_CTRL_KEYP_EN)) {
				kp->ctrl_reg |= KEYP_CTRL_KEYP_EN;
				pmic8058_kp_write_u8(kp, kp->ctrl_reg, KEYP_CTRL);
				pmic8058_kp_enable_irq(kp->key_sense_irq);
				pmic8058_kp_enable_irq(kp->key_stuck_irq);
			}
		}
	}
...
static int __devinit pmic8058_kp_probe(struct platform_device *pdev)
{
	...
	int rc;
	unsigned short *keycodes;
/* FUJITSU:2011-04-14 Del for OPEN_DET start */
//	u8 ctrl_val;
/* FUJITSU:2011-04-14 Del for OPEN_DET end */
	struct pm8058_chip	*pm_chip;
	...
	kp->pm_chip	= pm_chip;
/* FUJITSU:2011-04-14 Add for OPEN_DET start */
	kp->detect_irq = MSM_GPIO_TO_INT(40);
	kp->sw_lid = -1;
	kp->provisional_tmode = -1;
	kp->resume_done = 1;
	kp->wq = pmic8058_kp_create_workqueue("open_det");
	if (!kp->wq) {
		dev_err(&pdev->dev, "could not create a workqueue for open_det\n");
		rc = -ENOMEM;
		goto err_alloc_device;
	}
	pmic8058_kp_init_delayed_work(&kp->detect_work, pmic8058_kp_detect_work_fn);
	pmic8058_kp_wake_lock_init(&kp->wakelock, WAKE_LOCK_SUSPEND, "open_det");
/* FUJITSU:2011-04-14 Add for OPEN_DET end */
...
/* FUJITSU:2011-05-13 Add for OPEN_DET start */
	input_set_capability(kp->input, EV_SW, SW_LID);
/* FUJITSU:2011-05-13 Add for OPEN_DET end */
...
/* FUJITSU:2011-04-14 Add for OPEN_DET start */
	if (kp->wq)
		pmic8058_kp_destroy_workqueue(kp->wq);
/* FUJITSU:2011-04-14 Add for OPEN_DET end */
	kfree(kp);
	return rc;
}
...
/* FUJITSU:2011-04-15 Add for OPEN_DET start */
	device_unregister(kp->class_dev);
	if (kp->detect_irq_registered) {
		pmic8058_kp_disable_irq(kp->detect_irq);
		pmic8058_kp_free_irq(kp->detect_irq, NULL);
	}
/* FUJITSU:2011-04-15 Add for OPEN_DET end */
	free_irq(kp->key_stuck_irq, NULL);
	free_irq(kp->key_sense_irq, NULL);
	input_unregister_device(kp->input);
...
/* FUJITSU:2011-04-15 Add for OPEN_DET start */
	if (kp->wq) {
		pmic8058_kp_cancel_delayed_work(&kp->detect_work);
		pmic8058_kp_destroy_workqueue(kp->wq);
	}
/* FUJITSU:2011-04-15 Add for OPEN_DET end */
...
static int pmic8058_kp_suspend(struct device *dev)
{
	...
	pmic8058_kp_enable_irq_wake(kp->detect_irq);
/* FUJITSU:2011-05-30 Del for OPEN_DET start */
...
//--		mutex_unlock(&kp->mutex);
//--	}
/* FUJITSU:2011-05-30 Del for OPEN_DET end */
...
static int pmic8058_kp_resume(struct device *dev)
{
	...
	FUNCENTER("dev=%p", dev);
	pmic8058_kp_disable_irq_wake(kp->detect_irq);
	pmic8058_kp_disable_irq(kp->detect_irq);
	kp->sw_lid = -1;
	pmic8058_kp_wake_lock(&kp->wakelock);
	atomic_set(&kp->sample_cnt, 3);
...
{
/* FUJITSU:2011-04-18 Add for OPEN_DET start */
	keyboard_class = class_create(THIS_MODULE, "keyboard");
	if (IS_ERR(keyboard_class))
		return PTR_ERR(keyboard_class);
/* FUJITSU:2011-04-18 Add for OPEN_DET end */
